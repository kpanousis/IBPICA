...
class IBP_ICA:
    ...
    def global_params_VI(self):
        ...
        for k in range(self.K):
            self.t_f[k, 0] += self.rho * (
                self.f + 0.5 * (self.t_l[:, k].sum() + np.dot(self.t_mu[:, k].T, self.t_mu[:, k])))
        ...
        self.t_g_2 += self.rho * (
            self.gamma_2 - (psi(self.t_tau[:self.K - 1]) - psi(self.t_tau[:self.K - 1] + self.h_tau[self.K - 1])).sum())
    ...
    def getBatchGradients(self, x, s):
        ...
        # t_e_2 update
        batch_gradients[2] = (1.0 / self.S) * (
            self.eta_2 + 0.5 * self.N * self.zeta[s, :, :] * (self.t_s[s, :] + self.t_m[s, :] ** 2).reshape(-1, 1))
        ...
        for k in range(self.K):
            batch_gradients[5][:, k] = (self.N / self.S) * (self.t_a / self.t_b) * (batch_gradients[4][:, k]) * \
                                       self.t_m[s, k] * (x[s, :].T - np.dot(self.t_mu[:, np.arange(self.K) != k],
                                                                            self.t_m[s, np.arange(self.K) != k].T))
    ...
    def updateLocalParams(self, x, mb_indices):
        ...
        for k in range(self.K):
            self.t_s[mb_indices, k] = ((self.t_a / self.t_b) * (
                np.dot(self.t_mu[:, k].T, self.t_mu[:, k]) + self.t_l[:, k].sum()) \
                                       + (self.zeta[mb_indices, k, :] * (self.t_e_1[k, :] / self.t_e_2[k, :])).sum(
                1)) ** -1
        ...
        for n in range(self.S):
            for k in range(self.K):
                self.t_m[mb_indices[n], k] = (self.t_a / self.t_b) * (self.t_s[mb_indices[n], k]) \
                                             * (self.t_mu[:, k] * (
                    x[mb_indices[n], :]).T).sum()  # -np.dot(self.t_mu[:,k],self.t_m[mb_indices[n],k].T)).T).sum()

        for n in range(self.S):
            for k in range(self.K):
                self.zeta[mb_indices[n], k, :] = (
                    np.exp(psi(self.t_xi[k, :]) - psi(self.t_xi[k, :].sum()).reshape(-1, 1)
                           + 0.5 * (
                               psi(self.t_e_1[k, :]) - np.log(self.t_e_2[k, :]) - (
                                   self.t_e_1[k, :] / self.t_e_2[k, :]) * (
                                   self.t_m[mb_indices[n], k] ** 2 + self.t_s[mb_indices[n], k]))))
...
if __name__ == '__main__':
    ...
    x, s1, s2, s3,A = z.create_synthetic_data(dataset)
    # x=x[:1000,:]
    z.N = x.shape[0]
    z.D = x.shape[1]
    ...
    start_time = str(iteration) + (
        time.strftime("%Y-%m-%d-%H:%M").replace(" ", "_") + '_batch_size_' + str(initS) + '_D_' + str(
            z.D) + '_data_' + data)
    global_min = 0
    ...
    while iteration < max_iter:
        ...
        for miniBatch_indices in random_indices:
            ...
            if current_minibatch > 1:
                if abs(LL[iteration - 1, current_minibatch - 1] - LL[
                            iteration - 1, current_minibatch - 2]) < elbo_tolerance:
