...
class IBP_ICA:
    ...
    def init_params(self):
        ...
        # matrices
        #=======================================================================
        t_e_1=2.0*np.ones((self.K,self.J),dtype='float32')
        t_e_2=2.0*np.ones((self.K,self.J),dtype='float32')
        t_xi=2.0*np.ones((self.K,self.J),dtype='float32')
        t_l=2.0*np.ones((self.D,self.K),dtype='float32')
        t_mu=np.ones((self.D,self.K))
        omega=np.ones((self.D,self.K))
        t_s=2.0*np.ones((self.S,self.K),dtype='float32')
        ...
        zeta=1.0*np.ones((self.S,self.K,self.J))
...
#         self.batch_params=[t_b,t_e_1,t_e_2,t_xi,t_l,t_mu,t_a]
    ...
    def getBatchGradients(self,miniBatch,s):
        ...
        #tilde b update
        batch_gradients[0]=self.b
        for d in range(D):
            batch_gradients[0]+=0.5*(miniBatch[s,d]**2-2*miniBatch[s,d]*(self.batch_params[-2][d,:]*self.local_params[1][s,:]).sum()\
                                            +(self.batch_params[-2][d,:]*self.local_params[1][s,:]).sum()**2\
                                            +(((self.batch_params[-2][d,:]**2+self.batch_params[-3][d,:])*(self.local_params[1][s,:]**2+self.local_params[0])).sum())\
                                            -(self.batch_params[-2][d,:]**2*self.local_params[1][s,:]**2).sum()
                                            )
        
        #t_e_1 update
        batch_gradients[1]=self.eta_1+0.5*(self.local_params[2][s,:,:])
        
        #t_e_2 update
        batch_gradients[2]=self.eta_2+0.5*self.local_params[2][s,:,:]*(self.local_params[0][s,:]+self.local_params[1][s,:]**2).reshape(-1,1)
        
        #tilde xi update
        batch_gradients[3]=self.xi+self.local_params[2][s,:,:]
         
        #tilde lambda update
        batch_gradients[4]=np.zeros((self.D,self.K))
        for d in range(self.D):
            batch_gradients[4][d,:]=(self.batch_params[-1]/self.batch_params[0])*(self.local_params[0][s,:]+self.local_params[1][s,:]**2).reshape(-1,)\
                                     +(self.params[3]/self.params[4]).reshape(-1,)
        
        #tilde mu update
        batch_gradients[5]=np.zeros((self.D,self.K))
        for d in range(self.D):
            batch_gradients[5][d,:]=(batch_gradients[4][d,:]**(-1))*self.local_params[1][s,:]*(miniBatch[s,d].T-np.sum(self.batch_params[-2][d,:]*self.local_params[1][s,:]))
        
        #tilde a update
        batch_gradients[6]=self.a+0.5*self.D
...
#         self.batch_params=[t_b,t_e_1,t_e_2,t_xi,t_l,t_mu,t_a]
        ...
        self.local_params[0]*=(1-self.rho)
        self.local_params[0]+=self.rho*(((self.batch_params[-1]/self.batch_params[0])*(np.diag(np.dot(self.batch_params[-2].T,self.batch_params[-2])))\
                                        +(self.local_params[2]*(self.batch_params[1]/self.batch_params[2])).sum(2))**-1)

        self.local_params[1]*=(1-self.rho)
        self.local_params[1]+=self.rho*(self.local_params[0]*self.batch_params[-1]/self.batch_params[0]*(np.dot(self.batch_params[-2].T,miniBatch.T)).T)
        
        self.local_params[2]*=(1-self.rho)
        for n in range(self.S):
            self.local_params[2][n,:,:]+=self.rho*(np.exp(psi(self.batch_params[3])-psi(self.batch_params[3].sum(1)).reshape(-1,1)\
                                                   +0.5*(psi(self.batch_params[1])-np.log(self.batch_params[2])+(self.batch_params[1]/self.batch_params[2])*(self.local_params[1][n,:]**2+self.local_params[0][n,:]).reshape(-1,1))))
            self.local_params[2][n,:,:]/=self.local_params[2][n,:,:].sum(1).reshape(-1,1)
...
if __name__ == '__main__':
    ...
    start_time=str(iteration)+(time.strftime("%Y-%m-%d %H:%M").replace(" ","_"))
